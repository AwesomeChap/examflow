...
describe("exam attempt flow", () => {
  ...
  async function expireAttempt(examId: string) {
    // Push startedAt far enough back that any reasonable duration has elapsed.
  ...
  describe("submitting", () => {
    ...
    it("blocks answering after submission (409)", async () => {
      const { exam, mcq } = await makeExam();
      await studentAgent.post(`/exams/${exam.id}/attempt`);
  ...
  describe("backend-enforced time limit (auto-submit)", () => {
    it("rejects answers once the deadline has passed and auto-submits", async () => {
      ...
      assert.ok(state.body.attempt.submittedAt, "should be auto-submitted");
      assert.equal(state.body.attempt.score, 2, "graded on answers before the limit");
    });
